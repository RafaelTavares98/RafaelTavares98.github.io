...
WITH user_trips AS (
    SELECT 
        ...
        users.city,
        users.is_driver,
        COUNT(trips.trip_id) AS trip_id,
        SUM(trips.total_fare) AS total_fare
    FROM users
    -- FIX: Replace YOUR_ACTUAL_COLUMN_NAME with rider_id, driver_id, or whatever is in the trips table
    LEFT JOIN trips ON users.user_id = trips.YOUR_ACTUAL_COLUMN_NAME AND trips.status = 'completed'
    GROUP BY users.user_id, users.city, users.is_driver
)

SELECT 
    city,
    COUNT(user_id) AS user_id,
    SUM(is_driver) AS is_driver,
    SUM(trip_id) AS trip_id,
    COALESCE(ROUND(SUM(total_fare), 2), 0) AS total_fare
FROM user_trips
GROUP BY city
